...
class SportCatalogItem: TableAdapterItem {
  init(state: SportCatalogItem.State) {
    let cellData =  SportCatalogCellData(state: state)
    super.init(cellClass: SportCatalogCell.self, cellData: cellData)
  }
}

// MARK: - Data

class SportCatalogCellData: TableAdapterCellData {

  var state: SportCatalogItem.State
...
class SportCatalogCell: TableAdapterCell {
  
  public var data: SportCatalogCellData?
  ...
  override func fill(data: TableAdapterCellData?) {
    guard let data = data as? SportCatalogCellData else { return }
    self.data = data
